...
describe('TEST CalcSpecialDeductionOfSalary', () => {
  it('薪資特別扣除額 不足20w', () => {
    const amount = IndividualIncomeTax.CalcSpecialDeductionOfSalary(
      100000,
      25000
    )
...
describe('TEST CalcExemption', () => {
  it('免稅額 未滿70 * 1', () => {
    const amount = IndividualIncomeTax.CalcExemption(1, 0)
    assert.strictEqual(amount, 88000)
  })
  it('免稅額 超過70 * 1', () => {
    const amount = IndividualIncomeTax.CalcExemption(0, 1)
    assert.strictEqual(amount, 132000)
  })
  it('免稅額 未滿70 * 2 & 超過70 * 2', () => {
    const amount = IndividualIncomeTax.CalcExemption(2, 2)
...
describe('TEST CalcStandardDeduction', () => {
  it('標準扣除額 單身', () => {
    const amount = IndividualIncomeTax.CalcStandardDeduction('single')
    assert.strictEqual(amount, 120000)
  })
  it('標準扣除額 已婚', () => {
    const amount = IndividualIncomeTax.CalcStandardDeduction('married')
...
describe('TEST CalcBasicLivingExpenseDifference', () => {
  it('基本生活費差額 單身,有基本生活費', () => {
    const amount = IndividualIncomeTax.CalcBasicLivingExpenseDifference(
      2,
      1,
      'single'
    )
...
describe('TEST CalcProfessionalPractice9ATotal', () => {
  it('9A總計', () => {
    ...
    const amount = IndividualIncomeTax.CalcProfessionalPractice9ATotal(data)
...
describe('TEST CalcProfessionalPractice9BTotal', () => {
  it('9B總計&免稅額 非自行出版超過9B免稅額', () => {
    const amount = IndividualIncomeTax.CalcProfessionalPractice9BTotal(
      200000,
      500000
    )
...
describe('TEST taxBrackets', () => {
  it('稅率級距 1,210,001~2,420,000', () => {
    const amount = IndividualIncomeTax.taxBrackets(2420000)
...
describe('TEST CalcTotalIncomeTax', () => {
  it('綜合所得稅', () => {
    const amount = IndividualIncomeTax.CalcTotalIncomeTax(
      'single',
      0,
      3000000,
      400000,
